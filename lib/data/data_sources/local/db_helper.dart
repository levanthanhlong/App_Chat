...
class DatabaseHelper {
  ...
  Future<String?> hasUser() async {
    // kiểm tra có người dùng trong db không
    final db = await database;
    final List<Map<String, dynamic>> user =
        await db.query(ApiConstants.userTable, limit: 1);
    if (user.isNotEmpty) {
      return user.first[ApiConstants.token] as String?;
    }
    return null;
  }
  ...
  Future<List<FriendModel>> getAllFriends() async {
    ...
    return allFriends.map((friendMap) => FriendModel.fromMap(friendMap)).toList();
  }

  Future<List<FriendModel>> updateAllFriends(List<FriendModel> friendList) async {
  ...
  Future<void> insertMessages(String friendId, List<MessageModel> messages) async {
  ...
  Future<int> insertImage(String imageUrl, Uint8List image) async {
    // thêm ảnh vào bảng dữ liệu hình ảnh
    final db = await database;
    Map<String, dynamic> row = {
      ApiConstants.urlImage: imageUrl,
      ApiConstants.imageData: image,
    };
    return await db.insert(ApiConstants.imageDataTable, row);
  }

  Future<Uint8List> getImage(String imageUrl) async {
    // lấy ảnh từ dn ra, chỉ dùng lúc mất mạng
    final db = await database;
